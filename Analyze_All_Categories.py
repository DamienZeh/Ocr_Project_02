...
def all_data_books_in_category(link_first_page_category, name_category, path_csv, path_image): # écrit tous les livres de la categorie.
    ...
    for page in etl_pages_category(link_first_page_category):
        ...
        for books in etl_books_in_page(page):
            ...
            writer_books_category_csv(etl_book(books, soup_data_book), name_category, path_csv)
...
def etl_book(url_book, soup): #on récupère les détails du livre
    ...
    title = title_name[:90].replace(',', '_').replace("'", '_').replace(':', '_').replace('/', '_').replace('"',
                                                                                                            '_').replace(
        '*', '_') \
        .replace('?', '.').replace('#', '_').replace('%', '_').replace('-', '_').replace('é', 'e').replace('è', 'e') \
        .replace('à', 'a').replace('â', 'a').replace('â', 'a').replace(' ',
                                                                       '_')  # sans caractères spéciaux, limite taille
...
def writer_books_category_csv(data_category, name_category, path_csv):
    path_csv = 'Analyze_All_Categories__Csv'  # répertoire de sortie pour les images.
